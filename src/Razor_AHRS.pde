...
// Drift correction rotation matrix
...
void setup()
{
  ...
  /* Give first timestamp value */
  timestamp = millis();
  // Init sensors
  delay(100);  // Give sensors enough time to start
  sensors_init(accel, gyro, magneto);
  setup_SD();
  delay(1000);  // Give sensors enough time to start  
  // Init rotation matrix
  /* init_rotation_matrix(gyro, DCM_Matrix); */
}

boolean magneto_update, accel_update, gyro_update;
/* int accel_update = 0;  */
/* int gyro_update = 0; */
/* int magneto_update = 0; */
...
void loop()
{
  ...
  /*     read_control_message(curr_calibration_sensor,  */
  /*                          reset_calibration_session_flag,  */
  /*                          output_errors); */
  /* } */
  // Update sensor readings
  // Don't reset the sensor bit go low if the sensor reports it isn't ready
  // but let the sensor go high if it is ready
  accel_update = accel.read(); /*|| accel_update; */
  // If all sensors aren't ready then do not proceed any further, loop will start again
  /* if (!(accel_update)){ */
  /*   return; */
  /* } */
  /*  */
  /* output_sensors_text(accel, 'R'); */
  /* Send data to SD */
  stream_accel_to_SD(accel);
  /* Reset sensor bit */
  /* accel_update = false; */
}
